...
function checkVenvAndManager(backendDir: string, deepDiagnostics: boolean) {
  ...
  if (!fs.existsSync(venvPython)) {
    return {
      ...
      managerSpecCheckOutput: 'ModuSnap-ComfyUI-Backend/venv not found',
...
export async function GET(request: Request) {
  ...
  if (backendUp) {
    ...
    if (!managerEndpoint) {
      issues.push({
        ...
        fix: 'Restart backend via startup script (recommended):\n./start-backend.sh\n# direct launch (if needed):\npython3 main.py --enable-manager --enable-manager-legacy-ui --extra-model-paths-config extra_model_paths.yaml',
      });
  ...
  if (!managerRuntime.venvFound) {
    issues.push({
      ...
      cause: 'ModuSnap-ComfyUI-Backend/venv does not exist.',
    ...
    if (!managerRuntime.managerInstalled && !managerDetectedByRoutes) {
      issues.push({
        ...
        evidence: managerRuntime.managerSpecCheckOutput,
        fix: 'cd ModuSnap-ComfyUI-Backend\nsource venv/bin/activate\npip install -r manager_requirements.txt',
      });
    }
  ...
  if (logInfo.sslIssue) {
    issues.push({
      ...
      evidence: 'Detected CERTIFICATE_VERIFY_FAILED in ModuSnap-ComfyUI-Backend/user/comfyui.log',
      fix: 'ModuSnap-ComfyUI-Backend/venv/bin/python -m pip install --upgrade certifi\nexport SSL_CERT_FILE=$(ModuSnap-ComfyUI-Backend/venv/bin/python -c "import certifi; print(certifi.where())")\n# restart backend',
    });
  }
  ...
  const actions = issues.map((issue) => `${issue.title}: ${issue.fix}`);
    ...
    return NextResponse.json(
      {
        error: 'Failed to load manager diagnostics',
